...
public class TempFX extends Application {
  ...
  private LineChart<Number, Number> createBloodChart() {
    ...
    series.getData().add(new XYChart.Data(currentTimeMillis(), baselineTemp));
          ...
          runLater(
              () -> {
                ...
                if (values.length == 3) {
                  ...
                  series.getData().add(new XYChart.Data(currentTimeMillis(), bloodTemp.getValue()));
  ...
  private BorderPane loadMainUI() {
    ...
    rightVB.getChildren().addAll(indicator, sep, IndictorLevel3, IndictorLevel2, IndictorLevel1);
    ...
    connection.addListener(
        (ol, ov, nv) ->
            connStatusLbl.setText(
                nv ? "Connected on port ".concat(serial.getPortName()) : "Not connected!"));
    ...
    // Adding Listeners
    // sensorVal.addListener(
    //    (ol, ov, nv) -> sensorValueLbl.setText("Sensor Value: ".concat(nv.toString())));
    // volts.addListener((ol, ov, nv) -> vlotsLbl.setText("Volts: ".concat(nv.toString())));
