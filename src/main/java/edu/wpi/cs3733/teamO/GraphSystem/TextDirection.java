...
public class TextDirection {
    ...
    public static double direction(Node node1, Node node2, Node node3) {
        ...
        double angle = (180 / Math.PI) * Math.acos(dot_product / (v1Mag * v2Mag));    //angle between using dot-product

        // Determining cross Product
        double cross_product = (node2.getXCoord() * node3.getYCoord() - node2.getYCoord() * node3.getXCoord());
        ...
        for (int i = 0; i < path.size() - 2; i++) {
            ...
            if (angle < -25) {
                ...
                if (angle < -120) {
                    ...
                    Directions.add(
                            "Turn left from " + path.get(i + 1).getLongName() + " to " + path.get(i + 2).getLongName());

                } else {
                    Directions.add(
                            "Bear left from " + path.get(i + 1).getLongName() + " to " + path.get(i + 2).getLongName());
                }
                ...
                if (angle > 120) {
                    ...
                    Directions.add(
                            "Turn right from " + path.get(i + 1).getLongName() + " to " + path.get(i + 2).getLongName());

                } else {
                    Directions.add(
                            "Bear right from " + path.get(i + 1).getLongName() + " to " + path.get(i + 2).getLongName());
                }
            } else {
                Directions.add(
